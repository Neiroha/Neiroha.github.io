...
Neiroha 内置的 MiMo voice 候选会按模型名判断。普通 `mimo-v2-tts` 会提供 `mimo_default`、`default_zh`、`default_en`；v2.5 普通 TTS 模型会提供 `mimo_default`、`冰糖`、`茉莉`、`苏打`、`白桦`、`Mia`、`Chloe`、`Milo`、`Dean`。控制台文档更新后，以实际模型返回和官方说明为准。

## 声音设计写法

VoiceDesign 类模型没有固定 preset voice。角色里把任务模式切到声音设计，然后在 instruction 填写声音要求：
...
Dialog TTS、Phase TTS、Novel Reader、Video Dub 都会消耗同一个 key 的额度。长文本应先用短样本确认风格，再批量生成。
