...
云端 TTS 适合试用、跨设备使用，或暂不部署本地模型的场景。这里的“免费”通常指免费层、试用额度、赠送 credit 或限时活动，额度和模型可用性会变化，最终以官方控制台显示为准。

## 通用步骤

1. 到云服务官网创建账号和接口密钥（API Key）。
2. 在 Neiroha 打开 **提供商（Providers）**，点 **+**。
3. 选择对应适配器。
4. 填基础地址（`Base URL`）、接口密钥（`API Key`），必要时填模型名。
5. 点 **拉取全部（Fetch All）** 拉取模型和音色。
6. 打开启用开关。
7. 点 **健康检查（Health Check）**。
8. 在 **语音库（Voice Bank）** 创建角色，用快速测试生成一句短文本。
...
| 对话 TTS | 先选 2 到 3 行手动生成，避免一开始就全部生成 |
| 段落 TTS | 先拆 3 到 5 个段落，确认长文本风格和成本 |
| 小说阅读器 | 预取数量设低，避免一打开就消耗大量额度 |
| 视频配音 | 先导入短字幕片段测试 cue 对齐，再批量生成 |

## 提供商限流要填

云端服务最常见的失败并非配置错误，而是触发限流。推荐在提供商里设置：

| 字段 | 用途 |
|---|---|
| 最大并发 | 控制同时运行的 TTS 请求数量 |
| RPM | 每分钟请求数，适合 Gemini 这类明确限制 RPM 的服务 |
| TPM | 每分钟 token 数，适合按 token 计费或限制的服务 |
| RPD | 每日请求数，适合免费层 |

例如 Gemini 免费层可按官方 Rate Limits 给 `gemini-2.5-flash-preview-tts` 设置保守的 RPM / TPM / RPD；Azure F0 需优先按字符额度控制文本长度和批量规模；MiMo Token Plan 则以控制台余额和模型消耗规则为准。
