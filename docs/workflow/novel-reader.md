...
| 设置 | 建议值 |
|---|---|
| 最大切片字符 | `40` 到 `80`，先按模型稳定性调 |
| 跳过纯标点片段 | 开启 |
| 预取数量 | 云端免费层设 `1` 到 `2`，本地服务可逐步提高 |
| 自动翻页 | 需要连续听书时开启 |
| 自动推进章节 | 第一次测试先关闭，确认缓存逻辑后再开 |
...
云端免费层不要把预取和 Provider 并发都设很高。Novel Reader 会边读边补缓存，过高预取可能在短时间内消耗大量请求。
...
如果文本被重新切片，旧缓存不一定还能对应新片段。正式长篇项目建议先定好切片策略，再大量生成。
