...
语音角色是 Neiroha 的核心抽象。Provider 只说明服务连接方式；语音角色说明项目中的说话人、模型、音色、语速和风格。

<img className="screenshot" src="/img/screenshot_overview.png" alt="语音库页面" />

## 首次配置

1. 完成 [Provider 配置](/workflow/providers)，并确认 Provider 已启用。
2. 打开 **Voice Bank**。
...
如果下拉框缺少模型或音色，返回 Provider 页面点击 **Fetch All**。如果后端确实没有列表接口，则手动填模型名或 voice 名。

## 创建后立即测试

角色创建完成后，避免立即生成长文本。先在右侧 **Quick Test** 面板测试：

1. 选中刚创建的角色。
2. 在 Quick Test 输入一句短文本。
3. 点击生成按钮。
4. 能播放，说明 Provider、角色和队列均可用。
...
| 现象 | 处理 |
|---|---|
| 角色无法保存 | 检查必填字段，尤其是 Provider、任务模式、voice 或参考音频 |
| Provider 下拉框为空 | Provider 没启用，或当前平台不支持该 Provider |
| voice 下拉框为空 | 回 Provider 点 Fetch All，或手动填 voice |
| Quick Test 返回 401 / 403 | API Key 或云端区域错误 |
| Quick Test 返回 429 | 降低 Provider 并发和 RPM / RPD |
