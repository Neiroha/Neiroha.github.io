...
Provider 是 Neiroha 连接外部 TTS 后端的入口。它保存服务访问方式；项目实际使用的声音需要在 [语音角色与语音库](/workflow/voice-bank) 中绑定。
...
| 看到的情况 | 含义 | 处理 |
|---|---|---|
| 模型和音色都有 | 配置完整 | 打开启用开关，进入 Voice Bank |
...
| Health Check 失败 | URL、key、端口、网络或区域不正确 | 先用浏览器 / curl 验证后端，再修改 Provider |
...
| Azure F0 | 主要按字符额度和并发限制控制，长文本先用 Phase TTS 小批量验证 |
