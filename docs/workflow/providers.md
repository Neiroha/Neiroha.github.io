...
提供商是 Neiroha 连接外部 TTS 后端的入口。它保存服务访问方式；项目实际使用的声音需要在 [语音角色与语音库](/workflow/voice-bank) 中绑定。
...
1. 打开左侧导航的 **提供商（Providers）**。
2. 点击左侧提供商列表右上角的 **+**。
3. 选择适配器类型。本地模型接入见 [连接本地推理引擎](/workflow/providers/local-engines)；云端免费额度或试用额度接入见 [连接云端推理引擎](/workflow/providers/cloud-engines)。
4. 填名称。建议用 `平台 + 用途`，例如 `MiMo Free Trial`、`CosyVoice Local 9880`、`Azure East US`。
5. 填基础地址（`Base URL`）和接口密钥（`API Key`）。
6. 保存后回到提供商详情面板，先点 **拉取全部（Fetch All）**，再打开启用开关。
7. 点 **健康检查（Health Check）**。通过后再去创建语音角色。
...
提供商的并发、RPM、TPM、RPD 限流会作用到所有工作流：快速 TTS、对话 TTS、段落 TTS、小说阅读器、视频配音和本地 API 服务器都共用同一个 `TtsQueueService`。
...
| Azure F0 | 主要按字符额度和并发限制控制，长文本先用段落 TTS 小批量验证 |
| MiMo / 其他 Token Plan | 以控制台余额和速率限制为准，给提供商设置保守 RPD / TPM |
...
- 提供商通过后，继续 [创建语音角色与语音库](/workflow/voice-bank)。
